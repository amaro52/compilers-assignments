...
SYMBOL searchlev(char name[], int level) {
    SYMBOL sym;
    sym = symtab[level];
    while (sym != NULL && strcmp(name, sym->namestring) != 0) sym = sym->link;
    return sym;
}
...
void dbprsymbol(SYMBOL sym) {
    if (sym != NULL)
        printf(" %ld  %10s knd %1d %1d  typ %ld lvl %1d  siz %5d  off %5d lnk %ld\n", (long)sym,
               sym->namestring, sym->kind, sym->basicdt, (long)sym->datatype, sym->blocklevel,
               sym->size, sym->offset, (long)sym->link);
}
        ...
        case VARSYM:
            if (sym->datatype->kind == BASICTYPE)
                printf(" %ld  %10s  VAR    %1d typ %7s  lvl %2d  siz %5d  off %5d\n", (long)sym,
                       sym->namestring, sym->basicdt, sym->datatype->namestring, sym->blocklevel,
                       sym->size, sym->offset);
            else
                printf(" %ld  %10s  VAR    %1d typ %ld  lvl %2d  siz %5d  off %5d\n", (long)sym,
                       sym->namestring, sym->basicdt, (long)sym->datatype, sym->blocklevel,
                       sym->size, sym->offset);
            if (sym->datatype->kind != BASICTYPE) ppsym(sym->datatype);
            break;
        case TYPESYM:
            printf(" %ld  %10s  TYPE   typ %ld  lvl %2d  siz %5d  off %5d\n", (long)sym,
                   sym->namestring, (long)sym->datatype, sym->blocklevel, sym->size, sym->offset);
            if (sym->datatype->kind != BASICTYPE) ppsym(sym->datatype);
            break;
        case BASICTYPE:
            printf(" %ld  %10s  BASIC  basicdt %3d          siz %5d\n", (long)sym, sym->namestring,
                   sym->basicdt, sym->size);
            break;
        case SUBRANGE:
            printf(" %ld  %10s  SUBRA  typ %7d  val %5d .. %5d\n", (long)sym, sym->namestring,
                   sym->basicdt, sym->lowbound, sym->highbound);
            break;
        case CONSTSYM:
            switch (sym->basicdt) {
                case INTEGER:
                ...
                case REAL:
                    printf(" %ld  %10s  CONST  typ    REAL  val  %12e\n", (long)sym,
                           sym->namestring, sym->constval.realnum);
                    break;
                case STRINGTYPE:
                    printf(" %ld  %10s  CONST  typ  STRING  val  %12s\n", (long)sym,
                           sym->namestring, sym->constval.stringconst);
                    break;
            }
            break;
    };
