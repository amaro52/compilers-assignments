...
char* opprint[] = {" ",       "+",    "-",       "*",     "/",  ":=",   "=",     "<>",
                   "<",       "<=",   ">=",      ">",     "^",  ".",    "and",   "or",
                   "not",     "div",  "mod",     "in",    "if", "goto", "progn", "label",
                   "funcall", "aref", "program", "float", "fix"};
int opsize[] = {1, 1, 1, 1, 1, 2, 1, 2, 1, 2, 2, 1, 1, 1, 3,
                2, 3, 3, 3, 2, 2, 4, 5, 5, 7, 4, 7, 5, 3};
...
void debugprinttok(TOKEN tok) /* print a token for debugging */
{
    if (tok == NULL)
        ...
        printf(" token %ld  typ %2d  whic %3d  dty %3d  sty %ld lnk %ld  opnds %ld\n", (long)tok,
               tok->tokentype, tok->whichval, tok->basicdt, (long)tok->symtype, (long)tok->link,
               (long)tok->operands);
}
...
void printtok(TOKEN tok) /* print a token in abbreviated form */
{
    switch (tok->tokentype) {
        ...
        case RESERVED:
            ...
            break;
        default:
            printf("bad tokentype %d", tok->tokentype);
    }
}
...
void dbugprinttok(TOKEN tok) /* print a token in 'nice' debugging form */
{
    if (tok == NULL)
        ...
        switch (tok->tokentype) {
            case IDENTIFIERTOK:
                printf(" token %ld  ID  %12s  dtype %2d  link %ld\n", (long)tok, tok->stringval,
                       tok->basicdt, (long)tok->link);
                break;
            case STRINGTOK:
                ...
                switch (tok->basicdt) {
                    ...
                    case POINTER:
                        printf(" token %ld  NUM %12d  dtype %2d  link %ld\n", (long)tok,
                               tok->intval, tok->basicdt, (long)tok->link);
                        break;
                    case REAL:
                        printf(" token %ld  NUM %12e  dtype %2d  link %ld\n", (long)tok,
                               tok->realval, tok->basicdt, (long)tok->link);
                        break;
                };
                break;
            case OPERATOR:
                printf(" token %ld  OP  %12s  dtype %2d  link %ld  operands %ld\n", (long)tok,
                       opprint[tok->whichval], tok->basicdt, (long)tok->link, (long)tok->operands);
                break;
            case DELIMITER:
            case RESERVED:
                debugprinttok(tok);
...
void dbugbprinttok(TOKEN tok) /* print rest of token for debugging */
{
    if (tok != NULL)
        printf("  toktyp %6d  which  %6d  symtyp %ld  syment %ld  opnds %ld\n", tok->tokentype,
               tok->whichval, (long)tok->symtype, (long)tok->symentry, (long)tok->operands);
}
